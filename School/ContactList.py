...
def main_menu(contact_list):
    ...
    btn6 = Button(main,bg = "red", text="Exit Contacts", command= lambda: exit_list(contact_list, main))
...
def show_all(contact_list, main):
    ...
    for item in contact_list:
        ...
        show.pack(side=TOP,padx=tk_padx,pady=tk_pady, fill = X)
    btn1 = Button(showall, text="Exit", command= lambda: showall.destroy())
    btn1.pack(side=BOTTOM,padx=tk_padx,pady=tk_pady, fill = X)
...
def edit_contact(contact_list, main):
    ...
    def to_be_edited(number, edit):
        ...
        top.title('Add Contact')
        # Box 1
        Label(top, text="Enter your name:").pack(side=TOP,padx=tk_padx,pady=tk_pady, fill = X)
        ent = Entry(top)
