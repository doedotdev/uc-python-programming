...
class Calculator:
    ...
    def __init__(self, master):
        ...
        self.mult_button = Button(master, text="x", command=lambda: self.update("mult"))
        ...
        self.mult_button.grid(row=2, column=2)
    ...
    def update(self, method):
        if method == "add":
            ...
            if self.entered_number == 0:
                ...
                print("Cannot devide by zero, keeping number the same")
            else:
                self.total = self.total / self.entered_number
        else: # reset
            self.total = 0
